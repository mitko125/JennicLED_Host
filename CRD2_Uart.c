#include <stdio.h>
#include "avr_compiler.h"
#include "CRD2_Uart.h"
#include "usart.h"


#include "hardware.h"

#include "log.h"
#include "def.h"

#define DEBUG 0

int verbosity;

/* \brief  Receive buffer size: 2,4,8,16,32,64,128 or 256 bytes. */
//#define CRD2_RX_BUFFER_SIZE 1024
/* \brief Transmit buffer size: 2,4,8,16,32,64,128 or 256 bytes */
//#define CRD2_TX_BUFFER_SIZE 1024
/* \brief Receive buffer mask. */
#define USART_RX_BUFFER_MASK ( CRD2_RX_BUFFER_SIZE - 1 )
/* \brief Transmit buffer mask. */
#define USART_TX_BUFFER_MASK ( CRD2_TX_BUFFER_SIZE - 1 )


#if ( CRD2_RX_BUFFER_SIZE & USART_RX_BUFFER_MASK )
#error u8CRD2_RX_Buf buffer size is not a power of 2
#endif
#if ( CRD2_TX_BUFFER_SIZE & USART_TX_BUFFER_MASK )
#error u8CRD2_TX_Buf buffer size is not a power of 2
#endif


/* \brief Receive buffer. */
//static volatile uint8_t u8CRD2_RX_Buf[USART_RX_BUFFER_SIZE];
/* \brief Transmit buffer. */
//static volatile uint8_t u8CRD2_TX_Buf[USART_TX_BUFFER_SIZE];
/* \brief Receive buffer head. */
static volatile uint16_t RX_Head;
/* \brief Receive buffer tail. */
static volatile uint16_t RX_Tail;
/* \brief Transmit buffer head. */
static volatile uint16_t TX_Head;
/* \brief Transmit buffer tail. */
static volatile uint16_t TX_Tail;



uint8_t CRD2_kb_hit(void){
	return (RX_Head != RX_Tail);
}

int serial_read(unsigned char *data){
	
	if( RX_Head == RX_Tail)
		return 0;
	*data = (u8CRD2_RX_Buf[RX_Tail]);

	/* Advance buffer tail. */
	RX_Tail = (RX_Tail + 1) & USART_RX_BUFFER_MASK;
#if DEBUG
    if (verbosity >= LOG_DEBUG) daemon_log(LOG_DEBUG, "u8CRD2_RX_Buf %02x", *data);
#endif /* DEBUG */
	return 1;
}

uint8_t CRD2_get_char(void){
	uint8_t ans;

	while( RX_Head == RX_Tail);
	ans = (u8CRD2_RX_Buf[RX_Tail]);

	/* Advance buffer tail. */
	RX_Tail = (RX_Tail + 1) & USART_RX_BUFFER_MASK;

	return ans;
}

/*! \brief Test if there is data in the transmitter software buffer.
 *
 *  This function can be used to test if there is free space in the transmitter
 *  software buffer.
 *
 *  \param usart_data The USART_data_t struct instance.
 *
 *  \retval true      There is data in the receive buffer.
 *  \retval false     The receive buffer is empty.
 */
static bool USART_TXBuffer_FreeSpace(void)
{
	/* Make copies to make sure that volatile access is specified. */
	uint16_t tempHead = (TX_Head + 1) & USART_TX_BUFFER_MASK;
	uint16_t tempTail = TX_Tail;

	/* There are data left in the buffer unless Head and Tail are equal. */
	return (tempHead != tempTail);
}

int serial_write(uint8_t data){

#if DEBUG
    if (verbosity >= LOG_DEBUG) daemon_log(LOG_DEBUG, "u8CRD2_TX_Buf %02x", data);
#endif /* DEBUG */
	
	uint16_t tempTX_Head;

	while( !USART_TXBuffer_FreeSpace());

  u8CRD2_TX_Buf[tempTX_Head=TX_Head]= data;
	/* Advance buffer head. */
	TX_Head = (tempTX_Head + 1) & USART_TX_BUFFER_MASK;

	/* Enable DRE interrupt. */
	CRD2_uart.CTRLA = (CRD2_uart.CTRLA & ~USART_DREINTLVL_gm) | USART_DREINTLVL_LO_gc;
	
	
	return 0;
}

void CRD2_put_str(char *data){
	while( *data )
		serial_write( *data++ );
}

/*! \brief Receive complete interrupt service routine.
 */
ISR(CRD2_RX_vect)
{
	uint16_t tempRX_Head = (RX_Head + 1) & USART_RX_BUFFER_MASK;

	/* Check for overflow. */
	uint8_t data = CRD2_uart.DATA;

	if (tempRX_Head == RX_Tail) {
	  ;
	}else{
		u8CRD2_RX_Buf[RX_Head] = data;
		RX_Head = tempRX_Head;
	}
}

ISR(CRD2_TX_vect)
{
	;//CRD2_Port.OUTCLR = CRD2_DIR_PIN;
}

/*! \brief Data register empty  interrupt service routine.
 */
ISR(CRD2_DRE_vect)
{
	if (TX_Head == TX_Tail){
	    /* Disable DRE interrupts. */
		CRD2_uart.CTRLA = (CRD2_uart.CTRLA & ~USART_DREINTLVL_gm) | USART_DREINTLVL_OFF_gc;
	}else{
		//CRD2_Port.OUTSET = CRD2_DIR_PIN;
		/* Start transmitting. */
		CRD2_uart.DATA = u8CRD2_TX_Buf[TX_Tail];

		/* Advance buffer tail. */
		TX_Tail = (TX_Tail + 1) & USART_TX_BUFFER_MASK;
	}
}

void CRD2_UartInit(void){
	{
		/* This PORT setting is only valid to USARTC0 if other USARTs is used a
		* different PORT and/or pins is used. */
		/* PIN3 (TXD0) as output. */
		CRD2_Port.DIRSET = CRD2_TX_PIN;

		/* PC2 (RXD0) as input. */
		CRD2_Port.DIRCLR = CRD2_RX_PIN;
	
		/* PC1 (DIR) as output */
		//CRD2_Port.OUTCLR = CRD2_DIR_PIN;
		//CRD2_Port.DIRSET = CRD2_DIR_PIN;

		/* USARTC0, 8 Data bits, No Parity, 1 Stop bit. */
		USART_Format_Set(&CRD2_uart, USART_CHSIZE_8BIT_gc, USART_PMODE_DISABLED_gc, false);
		USART_Baudrate_Set(&CRD2_uart,(uint16_t)((F_CPU/16)/CRD2_BOUDE-1), 0);
	
		/* Enable RXC interrupt. */
		CRD2_uart.CTRLA = USART_RXCINTLVL_LO_gc | USART_TXCINTLVL_LO_gc;

		/* Enable both u8CRD2_RX_Buf and u8CRD2_TX_Buf. */
		CRD2_uart.CTRLB = 0;
		USART_Rx_Enable(&CRD2_uart);
		USART_Tx_Enable(&CRD2_uart);
	}
}